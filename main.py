...
class Collaborator(User):
    ...
    def save(self):
        with get_conn() as c:
            super().save()
            existing = c.execute("SELECT collab_id FROM collaborators WHERE collab_id = ?",
                                 (self.collab_id,)).fetchone()
            if existing:
                c.execute("UPDATE collaborators SET user_id = ?, position = ?, type = ? WHERE collab_id = ?",
                          (self.user_id, self.position, self.type, self.collab_id))
            else:
                c.execute("INSERT INTO collaborators (collab_id, user_id, position, type) VALUES (?,?,?,?)",
                          (self.collab_id, self.user_id, self.position, self.type))
            c.commit()
...
class Provider(User):
    ...
    def save(self):
        ...
        with get_conn() as c:
            exists = c.execute("SELECT provider_id FROM providers WHERE provider_id = ?",
                               (self.provider_id,)).fetchone()
            if exists:
    ...
    def load(provider_id: str) -> Optional["Provider"]:
        with get_conn() as c:
            ...
            if r:
                ...
                return Provider(name=user.name, phone=user.phone, user_id=user.user_id, provider_id=provider_id,
                                products=products)
            return None
...
class Client(User):
    ...
    def save(self):
        ...
        with get_conn() as c:
            ...
            if exists:
                c.execute("UPDATE clients SET sales = ?, type = ? WHERE client_id = ?",
                          (new_sales, self.type, self.client_id))
            else:
                c.execute("INSERT INTO clients (client_id, sales, type) VALUES (?,?,?)",
                          (self.client_id, self.sales, self.type))
            c.commit()
...
class Product:
    def __init__(self, name: str, types: str, desc: str, raw_p: float, sale_p: float, stock: int,
                 providers: Optional[List[str]] = None, prod_id: str = None):
    ...
    def save(self):
        ...
        with get_conn() as c:
            ...
            if exists:
                c.execute(
                    "UPDATE products SET name = ?, type = ?, providers = ?, description = ?, raw_price = ?, sale_price = ?, stock = ?",
                    (self.name, self.type, providers, self.description, self.raw_p, self.sale_p, self.stock))
            else:
                c.execute(
                    "INSERT INTO products (name, type, providers, description, raw_price, sale_price, stock) VALUES (?,?,?,?,?,?,?)",
                    (self.name, self.type, providers, self.description, self.raw_p, self.sale_p, self.stock))
            c.commit()
    ...
    def load(product_id: str) -> Optional["Product"]:
        with get_conn() as c:
            ...
            if r:
                ...
                return Product(name=r["name"], types=r["type"], desc=r["description"], raw_p=r["raw_price"],
                               sale_p=r["sale_price"], stock=r["stock"], providers=providers)
            return None


class Sales:
    def __init__(self, client_id: str, products: Optional[Dict[str, Dict[str, Any]]] = None, sale_id=None,
                 total: int = None, date_: str = None, time_: str = None):
    ...
    def save(self):
        with get_conn() as c:
            ...
            if exists:
                c.execute(
                    "UPDATE sales SET client_id = ?,date = ?, time = ?, products = ?, total = ? WHERE sale_id = ?",
                    (self._client_id, self.date, self.time, prod_json, self.total, self.sale_id))
            else:
                c.execute(
                    "INSERT INTO sales (sale_id, date, time, client_id, products, total) VALUES(?, ?, ?, ?, ?, ?)",
                    (self.sale_id, self.date, self.time, self._client_id, prod_json, self.total))
            c.commit()
