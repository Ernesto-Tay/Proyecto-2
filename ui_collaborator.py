...
class CollabUI(ctk.CTkFrame):
    ...
    def action(self, msg):
        ...
        match msg:
            ...
            case "Ver ventas":
                self.view_edit_sale("SAL0311152639")
            case "Agregar venta":
    ...
    def manage_sale(self, action=None, product_id=None, quantity=0, unit_price=0.0):
        ...
        if action == "add":
            ...
            if product_id in self.current_sale[
                "products"]:  # Acá, si el producto ya existe, suma cantidades y recalcula subtotal
        ...
        with get_conn() as conn:
            ...
            for product_id, data in self.current_sale["products"].items():
                ...
                if prod["stock"] < data["quantity"]:
                    mbox.showerror("Stock insuficiente",
                                   f"No hay suficiente stock para '{prod['name']}'.\n Disponible: {prod['stock']} unidades.")
                    return
            ...
            conn.execute(
                """INSERT INTO sales (sale_id, client_id, date, time, products, total)
                   VALUES (?, ?, ?, ?, ?, ?)""",
                (sale_id, self.current_sale["client"], date_str, time_str,
                 json.dumps(self.current_sale["products"], ensure_ascii=False), total))
    ...
    def update_product_search(self, *_):
        ...
        for row in rows:
            if term in row["name"].lower():
                ...
                add_btn = ctk.CTkButton(frame, text="Agregar", width=100,command=lambda pid=prod_id, p=price, q=qty_var, n=name: self.add_product(pid, p,
                                                                                                                 q, n))
